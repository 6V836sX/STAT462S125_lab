...
# discriminant score function for T class
delta_T <- function(x) {
  return(-(x - ____)^2/____ - log(____) + log(____))
}
```
...
df_test <- df_test %>% 
  rowwise() %>% 
  mutate(pred_by_hand = as.factor(ifelse(delta_yes(c(___, ___)) > delta_no(c(___, ___)), "Yes", "No")))
```
...
```{r, class.source="fill-in-blank"}
# True Positive count
TP <- sum(df_test$default == ____ & df_test$pred_by_hand == ____)
# True Negative count
TN <- sum(df_test$default == ____ & df_test$pred_by_hand == ____)
# False Negative count
FN <- sum(df_test$default == ____ & df_test$pred_by_hand == ____)
# False Positive count
FP <- sum(df_test$default == ____ & df_test$pred_by_hand == ____)
...
df_test <- df_test %>% 
  mutate(
    pred_qda = predict(____, newdata = ____)$class
  )
```
