...
We will split the dataset into a training set and a testing set with ratio of 8:2, you must be proficient at it now. Just remember to set a seed here so the result is consistent.
...
We try to find $\hat{b_0}$ and $\hat{b_1}$ such that the estimated $\hat{g_1(x)}$ yields a number close to one for "High" *medv_new* and a number close to zero for "Low" *medv_new*. This can be formalized into a *likelihood function*
...
If we apply logarithm to both side of the likelihood function, we shall get: $$ \log(\ell(b_0,b_1)) = \sum_i (y_i(b_0 + b_1 x_i) - \log\left(1 + \exp(b_0 + b_1 x_i)\right))$$
...
- `optim()` only return the best **par** before the maximum amount of iteration is reached, it may not be the overall optimiser.
:::
...
```
The minimiser within the provided range should locate in the dark region.
...
geom_point(aes(x=bhat[1], y=bhat[2]), color = "white")
```
...
There is no doubt that we have an existing function to find us the coefficients for the logistic regression. This time, it is `glm()` and we will need to set `family=binomial`. 
...
summary(logreg.fit)
```
*Do we have the same results from different approaches?*
...
```{r class.source="answer"}
table(df_test$medv_new, df_test$pred_class_by_hand)
```


```{r class.source="answer"}
library(caret)
confusionMatrix(as.factor(df_test$pred_class_by_hand), as.factor(as.logical(df_test$medv_new)))
```
...
- Recall(or, sensitivity, true positive rate), how many of the actual positive cases we were able to predict correctly $Recall = \frac{TP}{TP+FN}$
...
library(caret)
confusionMatrix(as.factor(as.numeric(df_test$pred_class_by_glm2)), as.factor(df_test$medv_new))
...
- Recall, sensitivity or true positive rate: 41/(41+3) = 0.9318(4dp)

What is the proportion that predicted positive cases are actually positive?

- Precision: 41/(41+17) = 0.7069(4dp)

What is the F1 score?

- 41\*2/(41\*2+17+3)=0.8039(4dp)

How is the performance compared to simple logistic regression accuracy-wise?

- The accuracy is about 5% higher than the simple logistic regression.
